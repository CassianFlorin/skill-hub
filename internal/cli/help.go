...
func englishHelp() helpMessages {
	return helpMessages{
		...
		Commands: []helpCommand{
			...
			{"publish", "Publish a local Skill into a registry"},
...
func englishTopics() map[string]helpTopic {
	return map[string]helpTopic{
		...
		"doctor":    {Usage: "usage: skillhub doctor [--json]", Description: "Show local config, runtime directories, registries, and installed Skill count.", Examples: []string{"  skillhub doctor", "  skillhub doctor --json"}},
		...
		"publish":   {Usage: publishUsage, Description: "Validate a local Skill package, copy it into a registry, and update skillhub.index.json. Git registries get an automatic commit and push; use --branch to push a review branch, or --pr to fork and open a pull request via gh.", Sections: []helpSection{{Title: "Options:", Lines: []string{"  --registry <name>   Target registry from skillhub config (required)", "  --dest <path>       Destination path inside the registry (default: existing entry path, skills/<namespace>/<name>, or <name>)", "  --trust <level>     Trust level for the entry (default: keep existing, or private)", "  --branch <name>     Push a new branch instead of the default branch (git registries)", "  --message <text>    Commit message (git registries)", "  --pr                Fork if needed and open a pull request via gh (git registries)", "  --dry-run           Show the index entry diff without writing", "  --json              Emit machine-readable output"}}}, Examples: []string{"  skillhub publish ./skills/git-commit-cn --registry company", "  skillhub publish ./skills/git-commit-cn --registry team --branch publish/git-commit-cn", "  skillhub publish ./skills/git-commit-cn --registry company --dry-run", "  skillhub publish ./skills/git-commit-cn --registry hub --pr"}},
		"history":   {Usage: "usage: skillhub history <identity> [--json]", Description: "List rollback snapshots saved before updates and reinstalls.", Examples: []string{"  skillhub history platform-team/java-review", "  skillhub history platform-team/java-review --json"}},
		...
		"holds":     {Usage: "usage: skillhub holds [--json]", Description: "List Skills currently held from update.", Examples: []string{"  skillhub holds", "  skillhub holds --json"}},
...
func simplifiedTopics() map[string]helpTopic {
	return map[string]helpTopic{
		...
		"doctor":    {Usage: "用法：skillhub doctor [--json]", Description: "显示本地配置、运行时目录、注册表和已安装 Skill 数量。", Examples: []string{"  skillhub doctor"}},
		...
		"publish":   {Usage: "用法：skillhub publish <skill-path> --registry <name> [--dest <path>] [--trust <level>] [--branch <name>] [--message <text>] [--pr] [--dry-run] [--json]", Description: "校验本地 Skill 包，复制到注册表并更新 skillhub.index.json。Git 注册表会自动 commit 并 push；使用 --branch 推送评审分支，或使用 --pr 通过 gh 自动 fork 并创建 Pull Request。", Sections: []helpSection{{Title: "选项：", Lines: []string{"  --registry <name>   目标注册表（必填）", "  --dest <path>       注册表内的目标路径（默认：已有条目路径、skills/<namespace>/<name> 或 <name>）", "  --trust <level>     条目信任级别（默认：保留已有值，否则 private）", "  --branch <name>     推送新分支而非默认分支（Git 注册表）", "  --message <text>    提交信息（Git 注册表）", "  --pr                必要时 fork 并通过 gh 创建 Pull Request（Git 注册表）", "  --dry-run           仅显示 index 条目差异，不写入", "  --json              输出机器可读结果"}}}, Examples: []string{"  skillhub publish ./skills/git-commit-cn --registry company", "  skillhub publish ./skills/git-commit-cn --registry team --branch publish/git-commit-cn", "  skillhub publish ./skills/git-commit-cn --registry company --dry-run", "  skillhub publish ./skills/git-commit-cn --registry hub --pr"}},
		"history":   {Usage: "用法：skillhub history <identity> [--json]", Description: "列出更新或重新安装前保存的回滚快照。", Examples: []string{"  skillhub history platform-team/java-review"}},
		...
		"holds":     {Usage: "用法：skillhub holds [--json]", Description: "列出当前被 hold 的 Skills。", Examples: []string{"  skillhub holds"}},
...
func traditionalTopics() map[string]helpTopic {
	return map[string]helpTopic{
		...
		"doctor":    {Usage: "用法：skillhub doctor [--json]", Description: "顯示本機設定、執行時目錄、註冊表和已安裝 Skill 數量。", Examples: []string{"  skillhub doctor"}},
		...
		"publish":   {Usage: "用法：skillhub publish <skill-path> --registry <name> [--dest <path>] [--trust <level>] [--branch <name>] [--message <text>] [--pr] [--dry-run] [--json]", Description: "校驗本地 Skill 套件，複製到註冊表並更新 skillhub.index.json。Git 註冊表會自動 commit 並 push；使用 --branch 推送評審分支，或使用 --pr 透過 gh 自動 fork 並建立 Pull Request。", Sections: []helpSection{{Title: "選項：", Lines: []string{"  --registry <name>   目標註冊表（必填）", "  --dest <path>       註冊表內的目標路徑（預設：既有條目路徑、skills/<namespace>/<name> 或 <name>）", "  --trust <level>     條目信任等級（預設：保留既有值，否則 private）", "  --branch <name>     推送新分支而非預設分支（Git 註冊表）", "  --message <text>    提交訊息（Git 註冊表）", "  --pr                必要時 fork 並透過 gh 建立 Pull Request（Git 註冊表）", "  --dry-run           僅顯示 index 條目差異，不寫入", "  --json              輸出機器可讀結果"}}}, Examples: []string{"  skillhub publish ./skills/git-commit-cn --registry company", "  skillhub publish ./skills/git-commit-cn --registry team --branch publish/git-commit-cn", "  skillhub publish ./skills/git-commit-cn --registry company --dry-run", "  skillhub publish ./skills/git-commit-cn --registry hub --pr"}},
		"history":   {Usage: "用法：skillhub history <identity> [--json]", Description: "列出更新或重新安裝前儲存的回滾快照。", Examples: []string{"  skillhub history platform-team/java-review"}},
		...
		"holds":     {Usage: "用法：skillhub holds [--json]", Description: "列出目前被 hold 的 Skills。", Examples: []string{"  skillhub holds"}},
