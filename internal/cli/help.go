...
func simplifiedTopics() map[string]helpTopic {
	return map[string]helpTopic{
		...
		"publish":   {Usage: "用法：skillhub publish <skill-path> --registry <name> [--dest <path>] [--trust <level>] [--branch <name>] [--message <text>] [--dry-run] [--json]", Description: "校验本地 Skill 包，复制到注册表并更新 skillhub.index.json。Git 注册表会自动 commit 并 push；使用 --branch 可推送评审分支而非默认分支。", Sections: []helpSection{{Title: "选项：", Lines: []string{"  --registry <name>   目标注册表（必填）", "  --dest <path>       注册表内的目标路径（默认：已有条目路径、skills/<namespace>/<name> 或 <name>）", "  --trust <level>     条目信任级别（默认：保留已有值，否则 private）", "  --branch <name>     推送新分支而非默认分支（Git 注册表）", "  --message <text>    提交信息（Git 注册表）", "  --dry-run           仅显示 index 条目差异，不写入", "  --json              输出机器可读结果"}}}, Examples: []string{"  skillhub publish ./skills/git-commit-cn --registry company", "  skillhub publish ./skills/git-commit-cn --registry team --branch publish/git-commit-cn", "  skillhub publish ./skills/git-commit-cn --registry company --dry-run"}},
...
func traditionalTopics() map[string]helpTopic {
	return map[string]helpTopic{
		...
		"publish":   {Usage: "用法：skillhub publish <skill-path> --registry <name> [--dest <path>] [--trust <level>] [--branch <name>] [--message <text>] [--dry-run] [--json]", Description: "校驗本地 Skill 套件，複製到註冊表並更新 skillhub.index.json。Git 註冊表會自動 commit 並 push；使用 --branch 可推送評審分支而非預設分支。", Sections: []helpSection{{Title: "選項：", Lines: []string{"  --registry <name>   目標註冊表（必填）", "  --dest <path>       註冊表內的目標路徑（預設：既有條目路徑、skills/<namespace>/<name> 或 <name>）", "  --trust <level>     條目信任等級（預設：保留既有值，否則 private）", "  --branch <name>     推送新分支而非預設分支（Git 註冊表）", "  --message <text>    提交訊息（Git 註冊表）", "  --dry-run           僅顯示 index 條目差異，不寫入", "  --json              輸出機器可讀結果"}}}, Examples: []string{"  skillhub publish ./skills/git-commit-cn --registry company", "  skillhub publish ./skills/git-commit-cn --registry team --branch publish/git-commit-cn", "  skillhub publish ./skills/git-commit-cn --registry company --dry-run"}},
